...
int main(int argc, char** argv) {
  ...
  shared_ptr<World> worldSharedPtr = make_shared<World>(w);
   ...
   * which is not how make_shared is intended to be used.
   * 
   * In fact, we are directly creating a shared_ptr<Robot> object and 
   * costructing a Robot object directly inside the make_shared function.
  */
  shared_ptr<Robot> robotSharedPtr = make_shared<Robot>(0.3, worldSharedPtr, robot_pose);

  /**
   * TO USE, IN CASE | FOR NOW LET'S USE A NORMAL "SHAREDPTR" WITHOUT DYNAMIC CAST
   * 
   * Here, we're trying to cast robotSharedPtr (which points to a Robot object) into a shared_ptr<WorldItem> (which is a base class of Robot).
   * With this being said, robotPointer will point to the same object as robotSharedPtr, but with the type shared_ptr<WorldItem>.
   * 
  ...
  */
  //shared_ptr<WorldItem> robotPointer = dynamic_pointer_cast<WorldItem>(robotSharedPtr);
